...
public:
    ...
    loadAsset(AAssetManager *assetManager, const std::string &assetPath){
        ...
        auto pAndroidPng = AAssetManager_open(
                assetManager,
                assetPath.c_str(),
                AASSET_MODE_BUFFER);
        ...
        auto result = AImageDecoder_createFromAAsset(pAndroidPng, &pAndroidDecoder);
    ...
    static std::shared_ptr <TextureAsset>
    makeAssetFromData(uint8_t * data, int width, int height){
    ...
    static std::shared_ptr <TextureAsset>
    make1DAssetFromData(uint8_t * data, int width, int height){
        GLuint textureId;
        glGenTextures(1, &textureId);
        glBindTexture(GL_TEXTURE_2D, textureId);
        ...
        glTexImage2D(
                GL_TEXTURE_2D, // target
                0, // mip level
                GL_LUMINANCE, // internal format, often advisable to use BGR
                width, // width of the texture
                height, // height of the texture
                0, // border (always 0)
                GL_LUMINANCE, // format
